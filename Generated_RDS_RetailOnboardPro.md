# Tài liệu Đặc tả Yêu cầu (RDS) - RetailOnboardPro

## I. Luồng Màn Hình (Screen Flow)

Đây là phần mô tả các luồng màn hình chính của ứng dụng cho các vai trò người dùng khác nhau.

### 1. Luồng màn hình chính (Main screen flow)
```markdown
a) Main screen flow 
...
### 2. Luồng màn hình của Nhân viên (Staff screen flow)
```markdown
b) Staff screen flow 
...
### 3. Luồng màn hình của Quản lý (Manager screen flow)
```markdown
...
## II. Trường Hợp Sử Dụng (Usecase)

Phần này mô tả chi tiết các trường hợp sử dụng (use cases) của hệ thống.

### 1. UC: Nhân viên Gửi Yêu Cầu (Staff Send Request)
```markdown
a. Diagram(s) Staff 
...
### 2. UC: Xem Danh Sách Yêu Cầu (View Request List - Staff)
```markdown
...
### 3. UC: Quản lý Xử Lý Yêu Cầu (Manager Manage Request)
```markdown
a. Diagram(s) Manager: 
...
### 4. UC: Quản lý Xem Lịch Sử Yêu Cầu (Manager View Request History)
```markdown
...
### 5. Chức năng chung: Đăng Nhập Hệ Thống (Login System)
```markdown
...
## III. Thiết Kế Màn Hình (Screen Design with Elements)

Phần này mô tả thiết kế giao diện người dùng (UI) và các yếu tố (elements) trên từng màn hình.
...
### 3. Màn hình: Quản lý Yêu Cầu (Manager side: Manage Request)
```markdown
...
### 4. Màn hình: Xem Lịch Sử Yêu Cầu (Manager side: View Request History)
```markdown
...
### 5. Màn hình: Gửi Yêu Cầu (Staff side: Send Request)
```markdown
...
### 6. Màn hình: Danh Sách Yêu Cầu (Staff side: Request List)
```markdown
